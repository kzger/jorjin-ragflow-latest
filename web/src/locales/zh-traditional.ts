export default {
  translation: {
    ...
    knowledgeDetails: {
      ...
      parsingStatus: '解析狀態',
      parsingStatusTip:
        '文件解析時間取決於多種因素。如果啟用了知識圖譜、RAPTOR、自動問題提取、自動關鍵詞提取等功能，解析時間將會顯著延長。如果解析進度條長時間未更新，請檢查工作日誌中的錯誤訊息。',
      ...
      rerankTip: `非必選項：若不選擇 rerank 模型，系統將默認採用關鍵詞相似度與向量餘弦相似度相結合的混合查詢方式；如果設定了 rerank 模型，則混合查詢中的向量相似度部分將被 rerank 打分替代。請注意：採用 rerank 模型會非常耗時。如需選用 rerank 模型，建議使用 SaaS 的 rerank 模型服務；如果你傾向使用本地部署的 rerank 模型，請務必確保你使用 docker-compose-gpu.yml 啟動 Jorjin RAG。`,
      topK: 'Top-K',
      ...
      enableChildrenDelimiter: '子文本塊用於檢索',
      childrenDelimiter: '文字分段標識符',
...
<p>編譯後的知識單元以分塊形式合併進分塊流輸出，適合在分塊文件之上構建可檢索的知識層。</p>`,
    ...
    <p>
      ...
      resume: `<p>支持的文件格式為<b>DOCX</b>、<b>PDF</b>、<b>TXT</b>。
        ...
        您只需與<i>'Jorjin RAG'</i>交談即可列出所有符合資格的候選人。
        </p>
          `,
...
以上就是你需要總結的內容。`,
 ...
 `,
      ...
      useGraphRag: '提取知識圖譜',
      useGraphRagTip:
        '基於知識庫內所有切好的文本塊構建知識圖譜，用以提升多跳和複雜問題回答的正確率。請注意：構建知識圖譜將消耗大量 token 和時間。',
      graphRagMethod: '方法',
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    setting: {
      ...
      ttsModel: '語音合成模型',
      ttsModelTip:
        '默認的tts模型會被用於在對話過程中請求語音生成時使用。如未显示可选模型，請確認模型供應商是否提供該模型。',
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '如未顯示可選模型，請檢查你是否在使用 Jorjin RAG slim 版（不含嵌入模型）；並確認模型供應商是否提供該模型。',
      img2txtModel: 'img2Txt模型',
      img2txtModelTip:
        '所有新創建的知識庫都將使用默認的 img2txt 模型。它可以描述圖片或視頻。如未顯示可選模型，請確認模型供應商是否提供該模型。',
      sequence2txtModel: 'speech2Txt模型',
      sequence2txtModelTip:
        '所有新創建的知識庫都將使用默認的 ASR 模型。使用此模型將語音翻譯為相應的文本。如未顯示可選模型，請確認模型供應商是否提供該模型。',
      rerankModel: 'rerank模型',
      rerankModelTip: `默認的 reranking 模型。如未顯示可選模型，請確認模型供應商是否提供該模型。`,
      workspace: '工作空間',
      upgrade: '升級',
      addLlmTitle: '添加Llm',
      modelName: '模型名稱',
      ...
      sitemapUserAgentTip:
        '每次請求送出的 User-Agent 標頭。留空則使用連接器預設值。',
      sitemapBatchSizeTip: '每批擷取並送到 Jorjin RAG 的頁面數量。',
